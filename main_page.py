...
if flag_keep_going:
    ...
    # st.write(spriv)
    # st.write(sprivb)
    dfjpriv = join_buf_w_df(mdf, sprivb, howjoin = "inner", pred = "intersects")
    dfjpub = join_buf_w_df(mdf, spubb, howjoin = "inner", pred = "intersects")
    # st.write(dfjpriv)
    ...
    for i, pn in enumerate(df["Product Name"]):
        st.write(str(i+ 1) +", Product: " +  pn + ", Quantity Used: " + str(df["Quantity Used"].iloc[i] )+ " " + df["Quantity Units"].iloc[i]+ ", Application Date: "+ str(df["Application Date"].iloc[i]))
#
